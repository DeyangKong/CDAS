...
def remote_extract_solution(solution_str):
...
def extract_solution_with_timeout(solution_str, timeout_duration=10):
    try:
        future = remote_extract_solution.remote(solution_str)
        result = ray.get(future, timeout=timeout_duration)
        return result
    except (GetTimeoutError, Exception) as e:
        ray.logger.info("Extraction timeout.")
        return None, False

@ray.remote(num_cpus=1)
def remote_qwen_math_equal(prediction, reference, timeout=False):
    return qwen_math_equal(prediction=prediction, reference=reference, timeout=timeout)

def qwen_math_equal_with_timeout_ray(prediction, reference, timeout_duration=10):
    try:
        future = remote_qwen_math_equal.remote(prediction, reference, timeout=False)
        result = ray.get(future, timeout=timeout_duration)
        return result
    except (GetTimeoutError, Exception) as e:
...
def compute_score(solution_str, ground_truth, method='strict', timeout_duration=10):
    """
    评分函数, 使用Ray超时机制对解答进行提取和比较。
    """
    extract_answer, is_boxed_matched = extract_solution_with_timeout(solution_str=solution_str, timeout_duration=timeout_duration)
    
    if extract_answer is None:
        return -1  # 超时的情况，返回一个负分
    
    correct = qwen_math_equal_with_timeout_ray(prediction=extract_answer, reference=ground_truth, timeout_duration=timeout_duration)
    ...
    elif reward_function_type == 'onlycorrect':
        if correct:
            box_match = 1.0
        else:
            box_match = 0.0    
...
def compute_independent_score(solution_str, ground_truth, method='strict', timeout_duration=10):
    """
    评分函数, 使用Ray超时机制对解答进行提取和比较。
    """
    extract_answer, is_boxed_matched = extract_solution_with_timeout(solution_str=solution_str, timeout_duration=timeout_duration)
...
def compute_remove_score(solution_str, ground_truth, method='strict', timeout_duration=10):
    """
    评分函数, 使用Ray超时机制对解答进行提取和比较。
    """
    extract_answer, is_boxed_matched = extract_solution_with_timeout(solution_str=solution_str, timeout_duration=timeout_duration)
