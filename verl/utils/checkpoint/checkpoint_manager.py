...
class BaseCheckpointManager:
    ...
    def remove_previous_save_local_path(self):
        ...
        print(f'[Rank {self.rank}] Checkpoint manager remove previous save local path: {abs_path}')
        if not os.path.exists(abs_path):
            return

        # Only remove checkpoint files for current rank
        rank_pattern = f"*_rank_{self.rank}.pt"
        rank_files = glob.glob(os.path.join(abs_path, rank_pattern))
        for file in rank_files:
            try:
                if os.path.exists(file):  # Double check existence before removal
                    os.remove(file)
                    print(f"[Rank {self.rank}] Removed checkpoint file: {file}")
            except OSError as e:
